...
class SMAP(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage is None:
            self.train_set = GenericDataset(
                data_frame = load_pickle_data(data_path / "SMAP/train.pkl", self.num_variables),
                window_size = self.window_size
            )

        if stage == "predict" or stage is None:
            self.test_labels = (
                load_pickle_data(data_path / "SMAP/test_label.pkl")
                .rename(columns={0: 'label'})
                .replace({True: 1, False: 0})
            )
            self.test_set = load_pickle_data(data_path / "SMAP/test.pkl", self.num_variables)

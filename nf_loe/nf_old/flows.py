...
class ActNorm(AffineConstantFlow):
    ...
    def forward(self, x):
        ...
        if not self.data_dep_init_done:
            ...
            x_std[x_std == 0] = 1e-10
            self.s.data = (-torch.log(x_std)).detach()
...
class AffineHalfFlow(nn.Module):
    ...
    def __init__(self, dim, parity, net_class=MLP, nh=24, scale=True, shift=True):
        ...
        self.parity = parity
        half_num = dim // 2
        self.s_cond = lambda x: x.new_zeros(x.size(0), half_num)
        self.t_cond = lambda x: x.new_zeros(x.size(0), half_num)
        if scale:
            self.s_cond = net_class(half_num, half_num, nh)
            if self.dim % 2:
                self.s_cond = net_class(dim-half_num, half_num, nh)
                if self.parity:
                    self.s_cond = net_class(half_num, dim-half_num, nh)
        if shift:
            self.t_cond = net_class(half_num, half_num, nh)
            if dim % 2:
                self.t_cond = net_class(dim-half_num, half_num, nh)
                if self.parity:
                    self.t_cond = net_class(half_num, dim-half_num, nh)
    def forward(self, x):
        x0, x1 = x[:,::2], x[:,1::2]
        # if self.dim % 2:
        #     x0, x1 = x[:,1::2], x[:,::2]
